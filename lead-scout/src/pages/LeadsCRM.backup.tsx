...
const LeadsCRM = () => {
    ...
    // We use a function in useState to only calculate this once on mount/initial render
    // However, since useIsMobile is a hook, it might not be ready immediately. 
    // Usually standard responsive design uses CSS, but here we are switching components.
    // Let's use an effect to sync if necessary, OR just rely on the hook's initial value if possible.
    // Since we want to respect user choice after load, we only set default.

    const [viewMode, setViewMode] = useState<ViewMode>(() => {
        return window.innerWidth < 768 ? 'list' : 'kanban';
    });

    // Optional: If you want it to react to window resizing LIVE (e.g. rotating a tablet), uncomment this:
    /*
    useEffect(() => {
        setViewMode(isMobile ? 'list' : 'kanban');
    }, [isMobile]);
    */
    // For now, let's keep it simple: initial load detection only, so we don't annoy users who manually switched.
    ...
    const handleLeadMove = (leadId: string, newStageId: string, newIndex: number) => {
        // Optimistic update would be complex without full backend support for reordering.
        // For now, simply update the stageId.
        // In a real app with order, we would calculated new order.
        updateLead(leadId, { stageId: newStageId });
        // Toast generic success if needed, or rely on global error handling
    };

    const handleSaveStages = (newStages: Stage[]) => {
        setStages(newStages);
        // Note: Stages are local only for now unless we add a backend endpoint for them.
    };
